...
use Onepix\FoodSpotVendor\Illuminate\Contracts\Support\Responsable;
...
class Uri implements Htmlable, Responsable, Stringable
{
    ...
    public function withQuery(array $query, bool $merge = true): static
    {
        ...
        if ($merge) {
            ...
            foreach ($query as $key => $value) {
                onepix_foodspotvendor_data_set($mergedQuery, $key, $value);
            }
            ...
            foreach ($query as $key => $value) {
                onepix_foodspotvendor_data_set($newQuery, $key, $value);
            }
        }
    ...
    {
        $currentValue = onepix_foodspotvendor_data_get($this->query()->all(), $key);
