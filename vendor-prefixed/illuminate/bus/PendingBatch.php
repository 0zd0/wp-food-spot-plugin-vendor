...
use Closure;
use Onepix\FoodSpotVendor\Illuminate\Bus\Events\BatchDispatched;
use Onepix\FoodSpotVendor\Illuminate\Contracts\Container\Container;
use Onepix\FoodSpotVendor\Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
...
class PendingBatch
{
    ...
    protected function ensureJobIsBatchable(object|array $job): void
    {
        foreach (Arr::wrap($job) as $job) {
            ...
            if (! (static::$batchableClasses[$job::class] ?? false) && ! in_array(Batchable::class, onepix_foodspotvendor_class_uses_recursive($job))) {
    ...
    {
        return onepix_foodspotvendor_value($boolean) ? $this->dispatch() : null;
    }
