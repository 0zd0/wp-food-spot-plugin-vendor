...
namespace Onepix\FoodSpotVendor\Carbon_Fields\Loader;

use Onepix\FoodSpotVendor\Carbon_Fields\Container\Repository as ContainerRepository;
...
use Onepix\FoodSpotVendor\Carbon_Fields\Libraries\Sidebar_Manager\Sidebar_Manager;
use Onepix\FoodSpotVendor\Carbon_Fields\Pimple\Container as PimpleContainer;
use Onepix\FoodSpotVendor\Carbon_Fields\Service\Legacy_Storage_Service_v_1_5;
use Onepix\FoodSpotVendor\Carbon_Fields\Service\Meta_Query_Service;
use Onepix\FoodSpotVendor\Carbon_Fields\Service\REST_API_Service;
...
class Loader {
	...
	public function boot() {
		...
		include_once( \Onepix\FoodSpotVendor\Carbon_Fields\DIR . '/core/functions.php' );
		...
		\Onepix\FoodSpotVendor\Carbon_Fields\Carbon_Fields::service( 'legacy_storage' )->enable();

		# Enable the meta query service
		\Onepix\FoodSpotVendor\Carbon_Fields\Carbon_Fields::service( 'meta_query' )->enable();

		# Enable the REST API service
		\Onepix\FoodSpotVendor\Carbon_Fields\Carbon_Fields::service( 'rest_api' )->enable();

		# Enable post meta revisions service
		\Onepix\FoodSpotVendor\Carbon_Fields\Carbon_Fields::service( 'revisions' )->enable();
	...
	public function load_textdomain() {
		$dir = \Onepix\FoodSpotVendor\Carbon_Fields\DIR . '/languages/';
	...
	protected function enqueue_style( $src, $deps = array() ) {
		...
		wp_enqueue_style(
			'carbon-fields-' . $src,
			\Onepix\FoodSpotVendor\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.css',
			$deps,
			\Onepix\FoodSpotVendor\Carbon_Fields\VERSION
		);
	}
	...
	protected function enqueue_script( $src, $deps = array() ) {
		...
		wp_enqueue_script(
			'carbon-fields-' . $src,
			\Onepix\FoodSpotVendor\Carbon_Fields\URL . '/build/' . $context . '/' . $src . $suffix . '.js',
			$deps,
			\Onepix\FoodSpotVendor\Carbon_Fields\VERSION
		);
	}
	...
	public function enqueue_assets() {
		...
		$revisions = \Onepix\FoodSpotVendor\Carbon_Fields\Carbon_Fields::service( 'revisions' );

		wp_localize_script( 'carbon-fields-vendor', 'cf', apply_filters( 'carbon_fields_config', array(
			'config' => array(
				'locale' => $this->get_ui_translations(),
				'pagenow' => $pagenow,
				'compactInput' => \Onepix\FoodSpotVendor\Carbon_Fields\COMPACT_INPUT,
				'compactInputKey' => \Onepix\FoodSpotVendor\Carbon_Fields\COMPACT_INPUT_KEY,
	...
	public function get_json_data() {
		...
		foreach ( $containers as $container ) {
			...
			if ( is_a($container, '\\Onepix\\FoodSpotVendor\\Carbon_Fields\\Container\\Block_Container', true ) ) {
	...
	public function fetch_association_options() {
		...
		/** @var \Onepix\FoodSpotVendor\Carbon_Fields\Field\Association_Field $field */
