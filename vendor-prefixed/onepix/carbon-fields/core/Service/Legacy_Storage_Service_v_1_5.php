...
namespace Onepix\FoodSpotVendor\Carbon_Fields\Service;

use Onepix\FoodSpotVendor\Carbon_Fields\Field\Field;
use Onepix\FoodSpotVendor\Carbon_Fields\Container\Container;
...
class Legacy_Storage_Service_v_1_5 extends Service {
	...
	protected function get_field_group_permutations( $fields ) {
		...
		foreach ( $fields as $field ) {
			if ( $field instanceof \Onepix\FoodSpotVendor\Carbon_Fields\Field\Complex_Field ) {
	...
	protected function get_table_details_for_datastore( Datastore_Interface $datastore ) {
		...
		if ( $datastore instanceof \Onepix\FoodSpotVendor\Carbon_Fields\Datastore\Theme_Options_Datastore ) {
			$details['prefix'] = '';
			$details['table_name'] = $wpdb->options;
			$details['table_key_column'] = 'option_name';
			$details['table_value_column'] = 'option_value';
		} else if ( $datastore instanceof \Onepix\FoodSpotVendor\Carbon_Fields\Datastore\Meta_Datastore ) {
	...
	protected function get_legacy_sql_comparisons_for_field( Field $field, $key_prefix, $key_column ) {
		...
		if ( $field instanceof \Onepix\FoodSpotVendor\Carbon_Fields\Field\Map_Field ) {
