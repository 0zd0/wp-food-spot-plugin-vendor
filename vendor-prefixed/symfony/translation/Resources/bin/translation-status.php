...
foreach ($config['original_files'] as $originalFilePath) {
    $translationFilePaths = onepix_foodspotvendor_findTranslationFiles($originalFilePath, $config['locale_to_analyze']);
    $translationStatus = onepix_foodspotvendor_calculateTranslationStatus($originalFilePath, $translationFilePaths);
    ...
    onepix_foodspotvendor_printTranslationStatus($originalFilePath, $translationStatus, $config['verbose_output'], $config['include_completed_languages']);
}

exit($totalTranslationMismatches > 0 ? 1 : 0);

function onepix_foodspotvendor_findTranslationFiles($originalFilePath, $localeToAnalyze): array
{
    $translations = [];
    ...
    foreach ($translationFiles as $filePath) {
        $locale = onepix_foodspotvendor_extractLocaleFromFilePath($filePath);
...
function onepix_foodspotvendor_calculateTranslationStatus($originalFilePath, $translationFilePaths): array
{
    $translationStatus = [];
    $allTranslationKeys = onepix_foodspotvendor_extractTranslationKeys($originalFilePath);

    foreach ($translationFilePaths as $locale => $translationPath) {
        $translatedKeys = onepix_foodspotvendor_extractTranslationKeys($translationPath);
        $missingKeys = array_diff_key($allTranslationKeys, $translatedKeys);
        $mismatches = onepix_foodspotvendor_findTransUnitMismatches($allTranslationKeys, $translatedKeys);
        ...
        $translationStatus[$locale]['is_completed'] = onepix_foodspotvendor_isTranslationCompleted($translationStatus[$locale]);
    }

    return $translationStatus;
}

function onepix_foodspotvendor_isTranslationCompleted(array $translationStatus): bool
{
    return $translationStatus['total'] === $translationStatus['translated'] && 0 === count($translationStatus['mismatches']);
}

function onepix_foodspotvendor_printTranslationStatus($originalFilePath, $translationStatus, $verboseOutput, $includeCompletedLanguages)
{
    onepix_foodspotvendor_printTitle($originalFilePath);
    onepix_foodspotvendor_printTable($translationStatus, $verboseOutput, $includeCompletedLanguages);
    echo \PHP_EOL.\PHP_EOL;
}

function onepix_foodspotvendor_extractLocaleFromFilePath($filePath)
{
...
function onepix_foodspotvendor_extractTranslationKeys($filePath): array
{
    $translationKeys = [];
 ...
 */
function onepix_foodspotvendor_findTransUnitMismatches(array $baseTranslationKeys, array $translatedKeys): array
{
    $mismatches = [];
...
function onepix_foodspotvendor_printTitle($title)
{
    echo $title.\PHP_EOL;
    echo str_repeat('=', strlen($title)).\PHP_EOL.\PHP_EOL;
}

function onepix_foodspotvendor_printTable($translations, $verboseOutput, bool $includeCompletedLanguages)
{
    ...
    foreach ($translations as $locale => $translation) {
        ...
        if ($translation['translated'] > $translation['total']) {
            onepix_foodspotvendor_textColorRed();
        } elseif (count($translation['mismatches']) > 0) {
            onepix_foodspotvendor_textColorRed();
        } elseif ($translation['is_completed']) {
            onepix_foodspotvendor_textColorGreen();
        }
        ...
        onepix_foodspotvendor_textColorNormal();
...
function onepix_foodspotvendor_textColorGreen()
{
    echo "\033[32m";
}

function onepix_foodspotvendor_textColorRed()
{
    echo "\033[31m";
}

function onepix_foodspotvendor_textColorNormal()
{
    echo "\033[0m";
}
