...
use Onepix\FoodSpotVendor\Symfony\Component\Console\Attribute\Argument;
use Onepix\FoodSpotVendor\Symfony\Component\Console\Attribute\Option;
    ...
    public function __invoke(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!\is_int($statusCode)) {
            if ($this->triggerDeprecations) {
                onepix_foodspotvendor_trigger_deprecation('symfony/console', '7.3', \sprintf('Returning a non-integer value from the command "%s" is deprecated and will throw an exception in Symfony 8.0.', $this->command->getName()));
    ...
    private function getParameters(InputInterface $input, OutputInterface $output): array
    {
        ...
        foreach ($this->reflection->getParameters() as $parameter) {
            ...
            if (!$type instanceof \ReflectionNamedType) {
                if ($this->triggerDeprecations) {
                    onepix_foodspotvendor_trigger_deprecation('symfony/console', '7.3', \sprintf('Omitting the type declaration for the parameter "$%s" is deprecated and will throw an exception in Symfony 8.0.', $parameter->getName()));
