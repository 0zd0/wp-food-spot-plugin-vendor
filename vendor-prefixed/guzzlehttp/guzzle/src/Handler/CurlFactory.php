...
namespace Onepix\FoodSpotVendor\GuzzleHttp\Handler;

use Onepix\FoodSpotVendor\GuzzleHttp\Exception\ConnectException;
use Onepix\FoodSpotVendor\GuzzleHttp\Exception\RequestException;
use Onepix\FoodSpotVendor\GuzzleHttp\Promise as P;
use Onepix\FoodSpotVendor\GuzzleHttp\Promise\FulfilledPromise;
use Onepix\FoodSpotVendor\GuzzleHttp\Promise\PromiseInterface;
use Onepix\FoodSpotVendor\GuzzleHttp\Psr7\LazyOpenStream;
use Onepix\FoodSpotVendor\GuzzleHttp\TransferStats;
use Onepix\FoodSpotVendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
