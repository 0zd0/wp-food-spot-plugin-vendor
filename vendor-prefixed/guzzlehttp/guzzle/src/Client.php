...
use Onepix\FoodSpotVendor\GuzzleHttp\Cookie\CookieJar;
use Onepix\FoodSpotVendor\GuzzleHttp\Exception\GuzzleException;
use Onepix\FoodSpotVendor\GuzzleHttp\Exception\InvalidArgumentException;
use Onepix\FoodSpotVendor\GuzzleHttp\Promise as P;
use Onepix\FoodSpotVendor\GuzzleHttp\Promise\PromiseInterface;
 ...
 * @final
 */
class Client implements ClientInterface, \Onepix\FoodSpotVendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        $uri = $this->buildUri(\Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::uriFor($uri), $options);
        if (\is_array($body)) {
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\UriResolver::resolve(\Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']), $uri);
        }
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::streamFor($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
        ...
        $request = \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
        if ($request->getBody() instanceof \Onepix\FoodSpotVendor\GuzzleHttp\Psr7\MultipartStream) {
