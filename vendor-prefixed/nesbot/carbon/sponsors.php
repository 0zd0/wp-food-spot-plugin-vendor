...
function onepix_foodspotvendor_getMaxHistoryMonthsByAmount($amount): int
{
    if ($amount >= 50) {
        return 6;
    }
...
function onepix_foodspotvendor_getHtmlAttribute($rawValue): string
{
    return str_replace(
        ['​', "\r"],
        '',
...
function onepix_foodspotvendor_getOpenCollectiveSponsors(): string
{
    ...
    foreach ($list as $member) {
        ...
        if (!str_contains($href, 'utm_source') && !preg_match('/^https?:\/\/(?:www\.)?(?:onlinekasyno-polis\.pl|zonaminecraft\.net|slotozilla\.com)(\/.*)?/', $href)) {
            $href .= (!str_contains($href, '?') ? '?' : '&amp;').'utm_source=opencollective&amp;utm_medium=github&amp;utm_campaign=Onepix\FoodSpotVendor\Carbon';
        }

        $title = onepix_foodspotvendor_getHtmlAttribute(($member['description'] ?? null) ?: $member['name']);
        $alt = onepix_foodspotvendor_getHtmlAttribute($member['name']);
