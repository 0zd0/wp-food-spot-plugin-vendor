...
abstract class ObjectRegistry implements Countable, IteratorAggregate
{
    ...
    public function load(string $name, array $config = []): object
    {
        ...
        if (isset($config['className'])) {
            if ($name === $config['className']) {
                [$plugin, $objName] = onepix_foodspotvendor_pluginSplit($name);
            } else {
                $objName = $name;
            }
        ...
        if (is_string($name)) {
            ...
            if ($className === null) {
                [$plugin, $name] = onepix_foodspotvendor_pluginSplit($name);
     ...
     * @return void
     * @throws \Onepix\FoodSpotVendor\Cake\Core\Exception\CakeException When a duplicate is found.
     */
     ...
     * @throws \Onepix\FoodSpotVendor\Cake\Core\Exception\CakeException If not loaded or found.
     * @phpstan-return TObject
     */
    ...
    public function normalizeArray(array $objects): array
    {
        ...
        foreach ($objects as $objectName => $config) {
            ...
            [$plugin, $name] = onepix_foodspotvendor_pluginSplit($objectName);
            if ($plugin) {
